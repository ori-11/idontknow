...
func get_BulletProperties_with_id(id:String, check_node:Node = get_tree().edited_scene_root):
	for n in check_node.get_children():
		if n is BulletProperties and n.id == id: return n
		for next in n.get_children():
			var res = get_BulletProperties_with_id(id, n)
